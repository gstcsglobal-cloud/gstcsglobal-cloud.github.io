...
(async () => {
  ...
  // ── 인트로(큐 축 66.0~70.0 · 표시 4.3s) — v16 «기념 문구 → CRT 꺼짐» 페이크아웃.
  //    ⚠ 표시(4.3s)가 큐(4.0)보다 길다: 영상시각 V 의 큐 = 66 + V/1.075. 아래 숫자는 전부 그걸로 환산했다.
  //    핵심은 꺼지는 순간 «험이 뚝 끊기는 것»이다 — 소리가 죽어야 화면이 죽은 걸로 믿는다.
  at(66.03, 'pop', { g: 0.15 }); at(66.07, 'thud');
  span(66.14, 69.53, 'hum', { f: 60, g: 0.04 });            // 전원 험 — 꺼짐(V3.80)에서 «뚝»
  at(66.96, 'tick', { g: 0.06, f: 1800 }); at(67.02, 'tick', { g: 0.05, f: 1200 });
  at(67.21, 'whoosh');
  at(67.72, 'blip', { f: 880, g: 0.07 });                    // 2001
  at(68.05, 'blip', { f: 1174, g: 0.08 });                   // -2026 (올라가는 두 음 = 그때→지금)
  at(67.95, 'thud'); at(68.00, 'bell', { n: 76, g: 0.09, d: 1.3 });   // 제목 착지
  at(68.45, 'blip', { f: 660, g: 0.05 });                    // 「참 많은 일이 있었습니다」
  at(68.95, 'tick', { g: 0.06, f: 3200 });                   // 마이크로 글리치
  at(69.53, 'zip'); at(69.75, 'thud');                       // 세로 붕괴 → 선이 점으로
  at(69.93, 'tick', { g: 0.05, f: 900 });                    // 점 소멸
