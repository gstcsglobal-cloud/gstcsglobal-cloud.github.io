...
(async () => {
  ...
  await page.goto(srv.url + '/' + (process.env.HTML || 'scene.html') + (process.env.QUERY || '')); await page.evaluate(() => document.fonts.ready);
  await page.evaluate(() => Promise.all(Array.from(document.images).map(i => i.complete ? null : new Promise(r => { i.onload = i.onerror = r; }))));
  const verr = await page.evaluate(() => window.ready || []); if (verr && verr.length) errs.push('VIDEO ' + JSON.stringify(verr));
  ...
  console.log(JSON.stringify({ frames: N, fonts: fontsOK, errors: errs.filter(e => !/ERR_FILE_NOT_FOUND/.test(e)).slice(0, 12) })); await browser.close(); srv.close();
})();
