...
> ⚠ **목록의 `DIGEST (SHA256)` 열은 시크릿 값이 아니다.** 값을 식별하려고 보여주는 해시다.
> 값은 그 행의 **눈 아이콘(값 보기)** 으로만 나온다. 다이제스트를 그대로 헤더에 붙여넣는 사고가
> 실제로 났다 — 64자 16진수라 시크릿처럼 생겼고, 틀려도 증상이 **403 하나뿐**이라
> "값이 틀렸나 · 헤더 이름이 틀렸나 · 시크릿이 아예 없나"를 가릴 수 없다.
...
**값이 틀렸을 때 가장 흔한 원인은 두 가지다.** 둘 다 눈으로 안 잡힌다:

- 넣은 값이 **정확히 64자 16진수**라면 Secrets 목록의 **DIGEST를 붙여넣었을** 확률이 높다(위 경고).
- `<SYNC_SECRET>` 같은 **자리표시자를 안 바꾼 채** 보냈을 수도 있다. `<...>` 꼴은 눈에 안 띈다 —
  실제로 cron 네 개가 통째로 그렇게 등록된 적이 있다.
...
#### 등록 직후 — 값이 제대로 들어갔는지 먼저 본다

`cron.schedule`은 **무엇을 넣든 성공한다.** 자리표시자가 그대로여도, 다이제스트를 넣어도
jobid를 돌려준다. 그래서 등록하자마자 이걸 돌린다:
