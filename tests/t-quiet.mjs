...
console.log('\n[13] 가동현황 표 — 행에 못 담은 설비를 버리지 않는지 (v122)');
{
  ...
  /* 인원·실적도 같은 통을 봐야 한다. 설비만 «기타»로 담고 인원은 버리면
     ...
     ⚠ 「생김새」가 아니라 「무엇에서 나오나」를 본다(v122 의 교훈). v132 에 판정이
       `_ALLK.has(k)` 한 줄에서 «이름 → 행 목록»(_rowsOf) 으로 바뀌었다 — 규칙은 그대로인데
       옛 정규식만 붉게 떴다. 지킬 것은 둘이고, 둘 다 «어디서 나오나»의 문제다.
     ⚠ 숫자가 맞는지는 소스로 못 본다 — 실제로 그려서 세는 것은 t-optbl.mjs 다. */
  is(/_cut\.forEach\(k=>_namesOf\(k\)\.forEach\(n=>_reg\(n,_ETC\)\)\)/.test(R),
  ...
  /* ⚠ 그렇다고 «설비 축에 아예 없는 값»을 기타로 몰면 안 된다 — 국내는 설비가
     ...
     설비에서 만든 목록(_rowsOf)에 없으면 «몰지 않고» 빈 값을 낸다. */
  is(/const cand=_rowsOf\[k\]; if\(!cand\)return ''/.test(R),
     'report — 설비 축에 아예 없는 값은 기타로 몰지 않는다 (v98 · 억지로 잇지 않는다)');
  /* ── 한 고객사가 «여러 법인»으로 갈릴 때 (v132 · 사용자 보고) ──────────────
     Micron 은 F16·F11·F16N 세 법인이 전부 정규화하면 MICRON 이다. 정규화 이름 하나에
     «첫 행»만 매어 두면 세 법인의 인원·실적이 가장 큰 행으로 몰리고 나머지는
     「자료없음」이 된다 — 에러 없이. 가르는 근거(FAB)를 실제로 넘기는지 본다.
     ⚠ 접근자만 정의하고 안 넘기면 이름 검사는 통과한다. 호출부를 직접 본다. */
  is(/bucket\(rkP\(p\), *_fabTok\(/.test(R), 'report — 인원을 담을 때 FAB 을 같이 넘긴다');
  is(/bucket\(rkW\(x\), *_fabTok\(/.test(R), 'report — 실적을 담을 때도 FAB 을 같이 넘긴다');
  /* 합치기(PSMC & TASC)는 이 표의 «행»에만 건다. GST.ORG.customer 를 고쳐서 합치면
     여덟 페이지의 고객사 축이 통째로 바뀌어 서로 다른 두 고객사가 한 값이 된다. */
  is(!/PSMC *& *TASC/.test(fs.readFileSync(ROOT+'/assets/core.js','utf8')),
     'core.js 의 고객사 축(GST.ORG.customer)에는 합치기가 들어가지 않는다');
