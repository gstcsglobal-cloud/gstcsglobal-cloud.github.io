...
console.log('\n[9] 여러 개를 동시에 골라도 둘 다 통과하는지 (실제 동작)');
{
  ...
  console.log('\n[9-4] 가동현황 표의 단계 판정 (v111)');
  {
    ...
    /* ⚠ «생김새»가 아니라 «무엇에서 나오나»를 본다(v122 의 교훈). 예전에는 한 줄짜리
       삼항식의 꼬리(`:_campOf(r))||'').trim()`)를 정규식으로 요구하고 있었다 — rkI 가
       여러 줄로 바뀌자 규칙은 그대로인데 검사만 붉게 떴다. 지킬 것은 하나다:
       단지 단계의 설비 행 키가 _campOf 에서 나온다. */
    is(/const rkI=[\s\S]{0,400}?_campOf\(r\)/.test(RC), 'report — 설비 행 키가 단지로 내려간다');
